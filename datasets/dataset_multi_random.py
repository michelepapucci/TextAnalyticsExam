...
def main():
    dataset = load_from_csv("dataset_multilabel_random.csv")
    list_of_sentences = []
    for row in dataset['train']:
        list_of_sentences.append({"translation": {"s": row['Sentence'], "t": row['Label']}})
    with open("dataset_multilabel_random.json", "a") as output:
        for row in dataset['train']:
            output.write(json.dumps({"translation": {"s": row['Sentence'], "t": row['Label']}}))
            output.write("\n")
